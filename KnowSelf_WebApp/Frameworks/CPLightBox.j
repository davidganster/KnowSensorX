...
- (void)runModalForWindow:(CPWindow)aWindow
{
    ...
    {
        var button = [_actionButtons objectAtIndex:i];
        [[_modalWindow contentView] addSubview:button];
    };
...
- (CGRect)resizedRect:(CGRect)aRect
{
    var imageFrame = aRect,
        windowFrame = [[CPPlatformWindow primaryPlatformWindow] contentBounds];

    if (CGRectGetWidth(imageFrame) > CGRectGetWidth(windowFrame))
    {
        console.log("bild zu breit");
        var ratio = CGRectGetWidth(imageFrame) / CGRectGetHeight(imageFrame);
        return CGRectMake(0, 0, CGRectGetWidth(windowFrame) - 50, (CGRectGetWidth(windowFrame) - 50) / ratio);
    }
    else if (CGRectGetHeight(imageFrame) > CGRectGetHeight(windowFrame))
    {
        console.log("bild zu hoch");
        var ratio = CGRectGetWidth(imageFrame) / CGRectGetHeight(imageFrame);
        return CGRectMake(0, 0, (CGRectGetHeight(windowFrame) - 50) * ratio, CGRectGetHeight(windowFrame) - 50);
    }
    else
    {
        return imageFrame;
...
- (void)setActionButtons:(CPArray)buttons
{
